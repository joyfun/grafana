...
type SkysparkExecutor struct {
	//*models.DataSource
	QueryParser    *SkysparkdbQueryParser
	ResponseParser *ResponseParser
	//HttpClient     *http.Client
}

func NewSkysparkExecutor(datasource *models.DataSource) (tsdb.TsdbQueryEndpoint, error) {
	return &SkysparkExecutor{
		QueryParser:    &SkysparkdbQueryParser{},
		ResponseParser: &ResponseParser{},
	}, nil
}
...
func init() {
	glog = log.New("tsdb.skyspark")
	tsdb.RegisterTsdbQueryEndpoint("skyspark", NewSkysparkExecutor)
}

func (e *SkysparkExecutor) Query(ctx context.Context, dsInfo *models.DataSource, tsdbQuery *tsdb.TsdbQuery) (*tsdb.Response, error) {
	...
	if setting.Env == setting.DEV {
		glog.Debug("Skysparkdb query", "raw query", rawQuery)
	}
	...
	if resp.StatusCode/100 != 2 {
		return nil, fmt.Errorf("Skysparkdb returned statuscode invalid status code: %v", resp.Status)
	}
...
func (e *SkysparkExecutor) getQuery(dsInfo *models.DataSource, queries []*tsdb.Query, context *tsdb.TsdbQuery) (*Query, error) {
...
func (e *SkysparkExecutor) createRequest(dsInfo *models.DataSource, query string) (*http.Request, error) {
	...
	glog.Debug("Skysparkdb request", "url", req.URL.String())
	return req, nil
}
