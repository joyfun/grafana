...
export class SkysparkLogsQueryField extends React.PureComponent<Props, State> {
  ...
  async componentDidMount() {
    ...
    try {
      const queryBuilder = new SkysparkQueryBuilder({ measurement: '', tags: [] }, datasource.database);
      ...
      for (let index = 0; index < influxMeasurements.length; index++) {
        ...
        const queryBuilder = new SkysparkQueryBuilder(
  ...
  onPairsChanged = (pairs: KeyValuePair[]) => {
    ...
    const queryModel = new SkysparkQueryModel(
      {
        ...query,
        resultFormat: 'table',
        groupBy: [],
